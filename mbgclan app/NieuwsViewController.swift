...
class NieuwsViewController: UIViewController, NSXMLParserDelegate {
    
    var arrParsedData = [Dictionary<String, String>]()
    var currentDataDictionary = Dictionary<String, String>()
    var currentElement = ""
    var foundCharacters = ""
    var url: NSURL! = NSURL(string: "http://www.mbgclan.nl/posts/index.rss")
    ...
    override func viewDidLoad() {
        ...
        let swipeRightOnNews  = UISwipeGestureRecognizer(target: self, action: "SwipeRightOnNews:")
        swipeRightOnNews.direction = UISwipeGestureRecognizerDirection.Right
        self.view.addGestureRecognizer(swipeRightOnNews)
        
        let swipeLeftOnNews = UISwipeGestureRecognizer(target: self, action: "SwipeLeftOnNews:")
        swipeLeftOnNews.direction = UISwipeGestureRecognizerDirection.Left
        self.view.addGestureRecognizer(swipeLeftOnNews)
        
        let swipeDownOnNews = UISwipeGestureRecognizer(target: self, action: "SwipeDownOnNews:")
        swipeDownOnNews.direction = UISwipeGestureRecognizerDirection.Down
        self.view.addGestureRecognizer(swipeDownOnNews)
    ...
    @IBAction func SwipeDownOnNews(sender: UISwipeGestureRecognizer) {
        newsitemnumber = 0
        startParsingWithContentsOfURL(url)
        DisplayNews()
    }
